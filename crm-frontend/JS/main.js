(async () => {
    ...
    // Сохранение клиента
    const saveLocalClient = async (obj) => {
        try {
    ...
    // Поиск клиента по ID
    const searchLocalClient = async (id) => {
        try {
            const clients = await getLocalClients();
    ...
    // Обновление клиента
    const patchLocalClient = async (obj, id) => {
        try {
            const clients = await getLocalClients();
    ...
    // Удаление клиента
    const deleteLocalClient = async (id) => {
        try {
    ...
    const initialClient = async () => {
        ...
        if (existingClients.length === 0) {
            // Добавляем начальные данные только если localStorage пуст
            for (const client of arrayClientsData) {
                // Добавляем поле fio, которое ожидается в createClient
                const clientWithFio = {
                    ...client,
                    fio: `${client.surname} ${client.name} ${client.lastName}`,
                };
                await saveLocalClient(clientWithFio);
            }
        }
        arrayClientsData = await getLocalClients(); // Обновляем данные
        render(arrayClientsData); // Перерисовываем таблицу
    };
    ...
    tableHeadThDateCreate.classList.add(
        tableThClass,
        "table__th-create",
        "sort-down"
    );
    tableHeadThDateChange.classList.add(
        tableThClass,
        "table__th-change",
        "sort-down"
    );
    ...
    const sortItems = [
        tableHeadThId,
        tableHeadThName,
        tableHeadThDateCreate,
        tableHeadThDateChange,
    ];
    ...
    buttonAdd.addEventListener("click", () => {
        ...
        modalClient.modalFormAdd.addEventListener("submit", async (e) => {
            ...
            for (let i = 0; i < typesContact.length; i++) {
                ...
                if (typesContact[i].textContent === "Телефон") {
                    if (valueContact[i].value.length !== 11) {
                        valueContact[i].classList.add("wrong");
                        modalClient.textWrong.classList.remove(
                            "visible-hidden"
                        );
                        modalClient.textWrong.textContent =
                    ...
                    if (
                        valueContact[i].value.includes("@") !== true ||
                        valueContact[i].value.includes(".") !== true
                    ) {
                        valueContact[i].classList.add("wrong");
                        modalClient.textWrong.classList.remove(
                        ...
                        modalClient.textWrong.textContent =
                            "Введите корректную почту (a-z, A-Z (@/.))";
                        return;
                    }
                }
            ...
            try {
                ...
                const serverDataObj = await saveLocalClient(clientObj);
                arrayClientsData = await getLocalClients(); // Обновляем данные
                render(arrayClientsData); // Перерисовываем таблицу
            } catch (error) {
                console.log(error);
    ...
    function createClient(client) {
        ...
        actionButtonChange.addEventListener("click", function () {
            try {
                ...
                if (client.contacts.length > 4) {
                    ...
                    modalClient.modalButtonAddContacts.classList.remove(
        ...
        actionButtonDelete.addEventListener("click", async function () {
            ...
            actionButtonDeleteSpinner.classList.add(
            ...
            modalDelete.modalDeleteButton.addEventListener(
                "click",
                async () => {
    ...
    const changeModaltClient = (client) => {
        ...
        modalClient.modalFormAdd.addEventListener("submit", async (e) => {
            ...
            for (let i = 0; i < typesContact.length; i++) {
                ...
                if (typesContact[i].textContent === "Телефон") {
                    if (
                        valueContact[i].value === "" ||
                        valueContact[i].value.length !== 11
                    ) {
                        valueContact[i].classList.add("wrong");
                        modalClient.textWrong.classList.remove(
                            "visible-hidden"
    ...
    const modalClients = (onClose) => {
        ...
        modalContactsWrapper.classList.add(
            "clients__contacts-wrapper",
            "contacts"
        );
        modalWindowAdd.classList.add(
            "modal__clients",
            "modal__clients--active"
        );
        ...
        modalInputLastnameSpan.classList.add(
            "form__span",
            "form__span-lastname"
        );
        ...
        modalButtonAddContacts.addEventListener("click", (e) => {
            e.preventDefault();
            const contactsItem =
                document.querySelectorAll(".contacts__wrapper");
    ...
    const createModalDelete = () => {
        ...
        modalDeleteWrapper.classList.add(
            "modal__delete",
    ...
    const addContacts = () => {
        ...
        buttonDeleteTooltip.classList.add(
            "tooltip",
            "contacts__tooltip-delete"
        );
    ...
    const searchClient = (clients) => {
        ...
        searchInput.addEventListener("input", () => {
            ...
            const foundItems = document.querySelectorAll(
                ".header__search-item"
            );
            const foundLinks = document.querySelectorAll(
                ".header__search-link"
            );
